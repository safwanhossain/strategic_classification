...
def train_model(X_train, y_train, X_test, y_test, strat, num_epochs=15):
    # Initialize the model, loss function, and optimizer
    feature_dim = X_train.shape[1]
    model = LinearClassifier(feature_dim, num_agents, strat)
...
def plot_all(train_strat, val, val_strat):
    # Plotting the train and validation error
    plt.plot(train_strat, label='Strategic Train Loss')
    plt.plot(val, label='Strat Val Loss (Non Strat Training)')
    plt.plot(val_strat, label='Strat Val Loss (Strat Training)')
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
...
if __name__ == "__main__":
    ...
    plot_all(train_strat, strat_val, strat_val_start)
